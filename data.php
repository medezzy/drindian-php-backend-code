...
switch($_GET["type"]){
    case "check_user":
        ...
        try {
            ...
            $password = utf8_encode($_POST['pass']);
            $deviceId = utf8_encode($_POST['deviceId']);
            ...
            $medical_users_db = $client->$drIndianUsersDB;
            $users_collection = $medical_users_db->$usersCollection;
            ...
            if($role == "admin"){
                ...
                $lastLogin = round(microtime(true)  )* 1000; // + (7 * 24 * 60 * 60))
                $records = $users_collection->findOneAndUpdate($check_arr, array('$set'=>array("lastLogin"=>$lastLogin)),
    ...
    case "sign_up":
        try {
            $sign_up_info = utf8_encode($_POST['sign_up_info']);
            $sign_up_info = stripslashes($sign_up_info);
            
            $sign_up_info = json_decode($sign_up_info,true);
            
            $username = $sign_up_info['username'];
            $mobile  = $sign_up_info['mobile'];
            ...
            $records = $users_collection->findOne(array('$or'=>$check_arr));
             ...
             if($records){
                ...
                $sign_up_info['lastLogin'] = ((round(microtime(true) ) + (7 * 24 * 60 * 60)))* 1000;
            
                $response = $users_collection->insertOne($sign_up_info);
            
                if($response){
                    login_success($sign_up_info,"Login Successful","admin");
                }else{
    ...
    case "add_bill_record":
        try{
            ...
            if($role == "admin"){
                ...
                $records = $users_collection->findOne($check_arr);
                //echo var_dump($records);
               addBill($username,$records,$bInfo);
    ...
    case "add_stock":
        try{
            ...
            if($role == "admin"){
                ...
                updateStock($username,$records,$sInfo);
    ...
    case "get_medicines":
        try{
            ...
            $role = utf8_encode($_GET['role']);
            $deviceId = utf8_encode($_GET['deviceId']);
            $skip = (int)utf8_encode($_GET['skip']);
            $limit = (int)utf8_encode($_GET['limit']);
            ...
            if($role == "admin"){
                ...
                if($records){
                    $client = new MongoDB\Client("mongodb://".$dbUser.":".$dbPwd."@localhost:27017/");    
                    $medicines_db = $client->$mediDBName;
                    $medicine_collection = $medicines_db->$medicineCollection;
                    $records = $medicine_collection->find(array(),array('skip'=>$skip,'limit'=>$limit));
...
function login_success($response,$msg,$role){
    //$response = array();
...
function addBill($username,$records,$bInfo){
     if($records){
        ...
        $client = new MongoDB\Client("mongodb://".$dbUser.":".$dbPwd."@localhost:27017/");       
        $userDatabaseSelected = $client->$userDatabase;
        $bills_collection = $userDatabaseSelected->$billColName;
        ...
        if($response){
            ...
            $stocks_collection = $userDatabaseSelected->$stockColName;
            ...
            foreach($billMedicinesList as $item){
                ...
                if($stock_records){
                    ...
                    //echo "after sort : ".json_encode($details_arr);
                    ...
                    $updateResult = $stocks_collection->findOneAndUpdate($c_arr,array('$set'=> $stock_records));
                }
            }
...
function updateStock($username,$records,$sInfo){
